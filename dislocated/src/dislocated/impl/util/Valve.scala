...
/* code is from
 * https://github.com/pjfanning/pekko-stream-contrib/blob/main/src/main/scala/org/apache/pekko/stream/contrib/Valve.scala
 * */

package com.github.foldcat.dislocated.impl.util.valve
...
import SwitchMode.Close
import SwitchMode.Open
...
/** Materializes into a [[Future]] of [[ValveSwitch]] which provides a
  ...
  */
final class Valve[A](mode: SwitchMode)
    extends GraphStageWithMaterializedValue[FlowShape[A, A], Future[
      ValveSwitch
    ]]:
  ...
  private class ValveGraphStageLogic(
    ...
    override def onPush(): Unit =
      if isOpen then push(out, grab(in))

    override def onPull(): Unit =
      if isOpen then pull(in)

    private def isOpen = mode == SwitchMode.Open

    override def preStart() =
      promise.success(switch)
  end ValveGraphStageLogic
end Valve
